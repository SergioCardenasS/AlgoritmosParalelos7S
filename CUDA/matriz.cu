...
__global__
void sum_mat(float* A,float* B,float* C,int rows,int cols)
{
...
__global__
void sum_mat_r(float* A,float* B,float* C,int rows,int cols)
{
	int index_rows=(blockDim.x*blockIdx.x)+threadIdx.x;
	if(index_rows<rows)
	{
		for(unsigned int i=0;i<cols;++i)
			A[index_rows*cols+i]=B[index_rows*cols+i]+C[index_rows*cols+i];
	}
}

__global__
void sum_mat_c(float* A,float* B,float* C,int rows,int cols)
{
